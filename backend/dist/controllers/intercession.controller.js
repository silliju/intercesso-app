...
exports.searchUsersForIntercession = exports.respondIntercessionRequest = exports.getSentRequests = exports.getPublicIntercessionRequests = exports.getIntercessionRequests = exports.createIntercessionRequest = void 0;
 ...
 * (없음)        → 개인 요청
 */
function encodeMessage(target_type, group_id, message) {
    const msg = message || '';
    if (target_type === 'public')
        return `[PUBLIC]${msg}`;
    if (target_type === 'group' && group_id)
        return `[GROUP:${group_id}]${msg}`;
    return msg;
}
function decodeMessage(rawMessage) {
    if (!rawMessage)
        return { target_type: 'individual', message: '' };
    if (rawMessage.startsWith('[PUBLIC]'))
        return { target_type: 'public', message: rawMessage.slice(8) };
    const groupMatch = rawMessage.match(/^\[GROUP:([^\]]+)\](.*)/s);
    if (groupMatch)
        return { target_type: 'group', group_id: groupMatch[1], message: groupMatch[2] };
 ...
 */
const createIntercessionRequest = async (req, res) => {
    try {
        const userId = req.user.userId;
        const { prayer_id, recipient_id, group_id, target_type = 'individual', message = '', priority = 'normal', } = req.body;
        if (!prayer_id) {
            (0, response_1.sendError)(res, '기도 ID는 필수입니다', 400, 'VALIDATION_ERROR');
            return;
        }
        if (target_type === 'individual' && !recipient_id) {
            (0, response_1.sendError)(res, '개인 요청은 대상자 ID가 필요합니다', 400, 'VALIDATION_ERROR');
            return;
        }
        if (target_type === 'group' && !group_id) {
            (0, response_1.sendError)(res, '그룹 요청은 그룹 ID가 필요합니다', 400, 'VALIDATION_ERROR');
            return;
        }
        // 요청자 닉네임 조회
        const { data: requesterUser } = await supabase_1.default
            .from('users')
            .select('nickname')
            .eq('id', userId)
            .single();
        ...
        if (target_type === 'group' && group_id) {
            ...
            if (!members || members.length === 0) {
                (0, response_1.sendError)(res, '그룹 멤버가 없습니다', 400, 'NO_MEMBERS');
                return;
            }
            ...
            const { error } = await supabase_1.default.from('intercession_requests').insert(rows);
            if (error) {
                console.error('그룹 중보기도 요청 오류:', error);
                (0, response_1.sendError)(res, '그룹 중보기도 요청 실패', 500, 'CREATE_ERROR');
                return;
            }
            // 알림 일괄 발송
        ...
        if (target_type === 'individual' && recipient_id) {
            ...
            if (existing && existing.length > 0) {
                (0, response_1.sendError)(res, '이미 같은 분에게 중보기도 요청을 보냈습니다', 409, 'DUPLICATE_REQUEST');
                return;
            }
        }
        ...
        const { data: request, error } = await supabase_1.default
            .from('intercession_requests')
            .insert(insertData)
            .select()
            .single();
        if (error) {
            ...
            (0, response_1.sendError)(res, '중보기도 요청 실패', 500, 'CREATE_ERROR');
            return;
        }
        // 개인 요청이면 알림 발송
        if (target_type === 'individual' && recipient_id) {
        ...
        (0, response_1.sendSuccess)(res, { ...request, target_type: decoded.target_type, group_id: decoded.group_id, message: decoded.message }, '중보기도 요청을 보냈습니다', 201);
    }
    catch (e) {
...
exports.createIntercessionRequest = createIntercessionRequest;
/** 받은 중보기도 요청 목록 (나에게 온 요청) */
const getIntercessionRequests = async (req, res) => {
    try {
        const userId = req.user.userId;
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 20;
        const offset = (page - 1) * limit;
        const { data: requests, count } = await supabase_1.default
            .from('intercession_requests')
            .select(`
        *,
      ...
      `, { count: 'exact' })
        ...
        (0, response_1.sendPaginated)(res, items, { page, limit, total: count || 0 });
    }
    catch (e) {
...
exports.getIntercessionRequests = getIntercessionRequests;
/** 전체공개 중보기도 요청 목록 */
const getPublicIntercessionRequests = async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 20;
        const offset = (page - 1) * limit;
...
exports.getPublicIntercessionRequests = getPublicIntercessionRequests;
/** 보낸 중보기도 요청 목록 */
const getSentRequests = async (req, res) => {
    try {
      ...
      `, { count: 'exact' })
        ...
        const seenPublicPrayers = new Set();
        const items = [];
        for (const r of (requests || [])) {
            const decoded = decodeMessage(r.message);
            if (decoded.target_type === 'public') {
                // 같은 prayer_id의 PUBLIC 요청은 1건만 포함
                if (seenPublicPrayers.has(r.prayer_id))
                    continue;
...
/** 요청 수락/거절 */
const respondIntercessionRequest = async (req, res) => {
    try {
        const userId = req.user.userId;
        const { requestId } = req.params;
        const { status } = req.body; // 'accepted' | 'rejected'
        if (!status || !['accepted', 'rejected'].includes(status)) {
            (0, response_1.sendError)(res, '유효한 상태값이 필요합니다 (accepted/rejected)', 400, 'VALIDATION_ERROR');
            return;
        }
        const { data: existing } = await supabase_1.default
            .from('intercession_requests')
            ...
            .single();
        if (!existing) {
            (0, response_1.sendError)(res, '요청을 찾을 수 없습니다', 404, 'NOT_FOUND');
            return;
        }
        ...
        const { data: request, error } = await supabase_1.default
            ...
            .single();
        if (error) {
            (0, response_1.sendError)(res, '응답 처리 실패', 500, 'UPDATE_ERROR');
            return;
        }
        // 요청자에게 수락 알림
        if (status === 'accepted') {
            const { data: respUser } = await supabase_1.default
        ...
        (0, response_1.sendSuccess)(res, { ...request, target_type: decoded.target_type, message: decoded.message }, `요청을 ${status === 'accepted' ? '수락' : '거절'}했습니다`);
    }
    catch (e) {
...
exports.respondIntercessionRequest = respondIntercessionRequest;
/** 사용자 검색 (개인 요청 대상자 선택용) */
const searchUsersForIntercession = async (req, res) => {
    try {
        const userId = req.user.userId;
...
exports.searchUsersForIntercession = searchUsersForIntercession;
//# sourceMappingURL=intercession.controller.js.map