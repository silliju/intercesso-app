...
exports.createChurch = exports.getChurchById = exports.searchChurches = void 0;
...
const searchChurches = async (req, res) => {
    try {
        ...
        if (error) {
            ...
            (0, response_1.sendError)(res, '교회 검색 중 오류가 발생했습니다', 500, 'SEARCH_ERROR');
            return;
        }
...
const getChurchById = async (req, res) => {
    try {
        ...
        if (isNaN(churchId)) {
            (0, response_1.sendError)(res, '유효하지 않은 교회 ID입니다', 400, 'VALIDATION_ERROR');
            return;
        }
        const { data, error } = await supabase_1.default
            .from('churches')
            ...
            .single();
        if (error || !data) {
            (0, response_1.sendError)(res, '교회를 찾을 수 없습니다', 404, 'NOT_FOUND');
            return;
        }
        (0, response_1.sendSuccess)(res, data);
    }
...
const createChurch = async (req, res) => {
    try {
        ...
        if (!body.name || !body.si_do || !body.si_gun_gu) {
            (0, response_1.sendError)(res, '교회명, 시/도, 시/군/구는 필수입니다', 400, 'VALIDATION_ERROR');
            return;
        }
        ...
        const { data: existing } = await supabase_1.default
            ...
            .maybeSingle();
        if (existing) {
            (0, response_1.sendSuccess)(res, existing, '이미 등록된 교회입니다. 해당 교회를 선택해 주세요.', 200);
            return;
        }
        const { data: inserted, error } = await supabase_1.default
            .from('churches')
            .insert({
            name: body.name,
        ...
        if (error) {
            if (error.code === '23505') {
                (0, response_1.sendError)(res, '같은 주소의 교회가 이미 등록되어 있습니다', 409, 'DUPLICATE_CHURCH');
                return;
            }
            console.error('church create error:', error);
            (0, response_1.sendError)(res, '교회 등록 중 오류가 발생했습니다', 500, 'CREATE_ERROR');
            return;
        }
        (0, response_1.sendSuccess)(res, inserted, '교회가 등록되었습니다', 201);
    }
    catch (err) {
        console.error(err);
        (0, response_1.sendError)(res, '서버 오류', 500, 'SERVER_ERROR');
