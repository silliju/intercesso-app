 ...
 */
async function findOrCreateSocialUser(email, nickname, provider, providerId) {
    ...
    try {
        ...
        if (id_token) {
            ...
            try {
                ...
                (0, response_1.sendError)(res, '구글 사용자 정보 조회 실패', 401, 'INVALID_GOOGLE_TOKEN');
                return;
            }
        }
        else {
            (0, response_1.sendError)(res, '구글 토큰이 필요합니다', 400, 'MISSING_TOKEN');
            return;
        }
        if (!verifiedEmail) {
            (0, response_1.sendError)(res, '구글 계정에서 이메일을 가져올 수 없습니다', 400, 'EMAIL_REQUIRED');
            return;
        }
        // ─── 사용자 조회 or 생성 ──────────────────────────────
        const user = await findOrCreateSocialUser(verifiedEmail, verifiedNickname, 'google', googleUserId);
        // ─── JWT 발급 ─────────────────────────────────────────
        const token = generateJWT(user.id, user.email);
        (0, response_1.sendSuccess)(res, { user, token }, '구글 로그인 성공');
    }
    catch (error) {
        console.error('[구글 로그인 오류]', error);
        (0, response_1.sendError)(res, '구글 로그인 처리 중 오류가 발생했습니다', 500, 'SERVER_ERROR');
    }
};
    ...
    try {
        ...
        if (!finalAccessToken && code) {
            try {
                ...
                if (!callbackUri) {
                    (0, response_1.sendError)(res, '카카오 redirect_uri가 필요합니다', 400, 'MISSING_REDIRECT_URI');
                    return;
                }
                ...
                (0, response_1.sendError)(res, '카카오 인가코드 처리 실패', 401, 'KAKAO_TOKEN_EXCHANGE_FAILED');
                return;
            }
        }
        if (!finalAccessToken) {
            (0, response_1.sendError)(res, '카카오 액세스 토큰 또는 인가코드가 필요합니다', 400, 'MISSING_TOKEN');
            return;
        }
        ...
        try {
            ...
            (0, response_1.sendError)(res, '유효하지 않은 카카오 토큰입니다', 401, 'INVALID_KAKAO_TOKEN');
            return;
        }
        ...
        // ─── 사용자 조회 or 생성 ──────────────────────────────
        const user = await findOrCreateSocialUser(kakaoEmail, kakaoNickname, 'kakao', kakaoUserId);
        // ─── JWT 발급 ─────────────────────────────────────────
        const token = generateJWT(user.id, user.email);
        (0, response_1.sendSuccess)(res, { user, token }, '카카오 로그인 성공');
    }
    catch (error) {
        console.error('[카카오 로그인 오류]', error);
        (0, response_1.sendError)(res, '카카오 로그인 처리 중 오류가 발생했습니다', 500, 'SERVER_ERROR');
    }
};
exports.kakaoLogin = kakaoLogin;
//# sourceMappingURL=social_auth.controller.js.map